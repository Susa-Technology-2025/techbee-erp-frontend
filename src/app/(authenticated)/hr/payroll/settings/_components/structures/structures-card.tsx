...
import { StructureCardProps } from './structure-type';
import GenericFieldRenderer from '@/components/smart-crud/GenericFieldRenderer';
import { getFieldConfigs } from './structure-field';
    ...
    >
      ...
      >
        <Box sx={{ width: '100%' }}>
          <GenericFieldRenderer
            field={fieldConfigs[0]} // name field
        ...
        <GenericFieldRenderer
          field={fieldConfigs[1]} // code field
          value={editingValues[card.id!]?.code ?? card.code}
          onChange={(val: any) => onEditFieldChange(card.id!, 'code', val)}
          editing={card.editing}
          viewProps={{ sx: { fontWeight: 200, textTransform: 'uppercase', letterSpacing: 0.5, width: '100%',py:0.4, borderRadius: 10 } }}
      ...
      <Box sx={{ p: 2 }}>
        ...
        <GenericFieldRenderer
          ...
          multiline={true}
          minRows={3}
          viewProps={{ sx: { color: '#4a5568', fontSize: 15, lineHeight: 1.7, minHeight: 60 } }}
          editProps={{ variant: 'outlined', size: 'small', sx: { mb: 1, borderRadius: 2 }, fullWidth: true }}
        />
      </Box>
      <Box sx={{ p: 2, pt: 0 }}>
        ...
        <GenericFieldRenderer
          field={fieldConfigs[3]} // organizationNodeId field
          value={editingValues[card.id!]?.organizationNodeId ? card.organizationNodeId : 'N/A'}
          onChange={(val: any) => onEditFieldChange(card.id!, 'organizationNodeId', val)}
          editing={card.editing}
          viewProps={{ sx: { color: '#4a5568', fontSize: 15, lineHeight: 1.7 } }}
          editProps={{ variant: 'outlined', size: 'small', sx: { mb: 1, borderRadius: 2 }, fullWidth: true }}
        />
      </Box>
