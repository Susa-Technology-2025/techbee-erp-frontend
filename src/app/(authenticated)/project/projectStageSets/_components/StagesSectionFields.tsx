    ...
    <Controller
      ...
      render={({ field: controllerField, fieldState }) => (
        <TextField
          ...
          helperText={fieldState.error ? "Project stage code is required" : "The code for the project stage."}
          error={!!fieldState.error}
          disabled={false}
          className=""
           slotProps={{
...
export const StageSequenceField = ({index}: any) => {
  const { control: formControl } = useFormContext();
  return (
    <Controller
      ...
      render={({ field: controllerField, fieldState }) => (
        <TextField
          ...
          helperText={fieldState.error ? "Sequence is required" : "The sequence order of the project stage."}
          error={!!fieldState.error}
          disabled={false}
          className=""
           slotProps={{
      ...
      render={({ field: controllerField, fieldState }) => (
        <FormControl component="fieldset" className="">
          ...
          <FormHelperText error={!!fieldState.error}>
            {fieldState.error ? "Active status is required" : "Indicates if the project stage is active."}
          </FormHelperText>
        </FormControl>
      )}
    />
...
export const StageRequiresApprovalField = ({index}: any) => {
  const { control: formControl } = useFormContext();
  return (
    <Controller
      ...
      render={({ field: controllerField, fieldState }) => (
        <FormControl component="fieldset" className="">
          ...
          />
          <FormHelperText error={!!fieldState.error}>
            {fieldState.error ? "Requires approval status is required" : "Indicates if the project stage requires approval."}
          </FormHelperText>
        </FormControl>
      )}
    />
...
export const StageTriggersNotificationField = ({index}: any) => {
  const { control: formControl } = useFormContext();
  return (
    <Controller
      ...
      render={({ field: controllerField, fieldState }) => (
        <FormControl component="fieldset" className="">
          ...
          />
          <FormHelperText error={!!fieldState.error}>
            {fieldState.error ? "Triggers notification status is required" : "Indicates if the project stage triggers notification."}
          </FormHelperText>
        </FormControl>
      )}
    />
...
export const ProjectStagesArrayField = ({index}: any) => {
...
const [popoverAnchor, setPopoverAnchor] = useState<HTMLElement | null>(null);
      ...
      const { mutate, isPending } = useDataMutation({
        apiEndPoint: deleteMode?.id
          ? deleteMode.mode === "delete" ?"https://api.techbee.et/api/project/projectStages/" + deleteMode?.id: "https://api.techbee.et/api/project/projectStageSets/" + getValues("id") + "/stages"
          : "",
        method: "DELETE",
          invalidateQueryKey: [
      "data",
      "https://api.techbee.et/api/project/projectStages",
      "https://api.techbee.et/api/project/projectStageSets",
    ],
        onSuccess: (message) => {
  ...
  <>
    ...
    >
      ...
      <Stack spacing={1.5}>
        ...
        <StageSequenceField index={index} />
        <StageActiveField index={index} />
        <StageRequiresApprovalField index={index} />
        <StageTriggersNotificationField index={index} />
            <IconButton 
              color="error" 
